...
import {
  RUM_VIEWS_OPTIONS,
  RumViewsOptionACurrent,
  RumViewsOptionBSingleWorkbench,
  RumViewsOptionCDualWorkbench,
  RumViewsOptionDCardGrid,
} from './design-compare/rum-views-page-effects';
    ...
    <div className="grid gap-6 bg-[var(--color-fill-1)] p-4">
      <div className="rounded-lg border border-[var(--color-border-1)] bg-[var(--color-bg)] p-4">
        ...
        <p className="mb-0 mt-1.5 text-[13px] leading-relaxed text-[var(--color-text-3)]">
          对齐 OpsPilot Look B 卡与 Studio Workbench，不另发明皮肤。请在 Storybook 里对比后告诉我选
          A/B/C/D（可与会话页同选或分开选）。选定后再改生产页。
        </p>
        <ul className="mb-0 mt-3 list-disc space-y-1 pl-5 text-[13px] text-[var(--color-text-2)]">
          {RUM_VIEWS_OPTIONS.map((opt) => (
            <li key={opt.id}>
...
const meta = {
  title: 'Design/RUM Views Page Options',
  component: Gallery,
  parameters: {
    layout: 'fullscreen',
    docs: {
      description: {
        component:
          'RUM `/rum/views` 布局选型示意。B/C/D 对齐 OpsPilot 工作台与 Look B；A 为当前对照。Story only。',
      },
    },
  },
