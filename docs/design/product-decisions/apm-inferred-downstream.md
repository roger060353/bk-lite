...
- 最近更新：2026-09-01
- 当前规格：`specs/changes/apm-topology-ops-followup/spec.md`、`specs/changes/apm-inferred-downstream/spec.md`

## 产品定位

给 Web 等已插桩服务装一个探针后，运维能在 **应用详情拓扑** 上看到它访问过的 **MySQL / Redis / Kafka 等未插桩下游**，并下钻到产生该判断的 Client Span。这不是把 mysql 登记成 **APM 服务**，也不是 CMDB 资产发现。
...
MVP 做成 **应用详情画布上的推断节点 + 调用链可解释**，不扩张服务目录，也不把推断节点挂到全局服务拓扑。这样能验证「一个探针看见下游」的闭环，又不会污染应用列表、权限和组织继承。
...
- **推断节点只挂应用详情画布。** 全局服务拓扑 `include_inferred=false`。应用详情只展示本应用已插桩服务的直接推断下游，不进下属服务表 / KPI / 目录，避免 mysql 被理解成 APM 服务。一跳邻居自己的推断下游不进入本应用图。

## 用户闭环（MVP）

1. 用户给 Web 服务接入官方探针（系统内下载、钉死版本）。
2. Web 产生访问 MySQL / Redis 的 Client Span，经现有 OTC → NATS → VT。
3. 打开 **应用详情** 画布（可带现有时间窗）。全局服务拓扑不出现推断节点。
4. 图上除本应用已插桩服务外，出现带「推断」角标的直接 mysql / redis 下游。
...
- Web：仅应用详情画布区分推断节点样式；全局服务拓扑不请求推断。调查栏对推断节点隐藏服务详情入口。
...
- 「MVP 同时改服务拓扑和应用详情子拓扑」：2026-08-26 改为第一版只做服务拓扑。2026-08-28 用户确认应用详情**画布**补本应用的直接推断下游，仍不进目录 / KPI / 下属服务表。2026-09-01 改为推断节点只在应用详情呈现，全局服务拓扑不再展示。

## 决策来源

- 2026-09-01 用户确认：推断服务只在应用拓扑中呈现，全局服务拓扑不展示。
