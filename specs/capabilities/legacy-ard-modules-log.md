...
- 空处理人的活跃告警可认领 / 分派；当前处理人可转派已有处理人的活跃告警。手工关闭：没有处理人时具备操作权限即可关；已有处理人时只有当前处理人（id 或 username）能关，其他人 409 且状态不变。策略删除关闭不看处理人。认领、分派、转派、关闭写入 Event。`my_alert` 在当前可见集合上再筛处理人（主列表、`/all`、stats 同一套），不靠 `operator`。
