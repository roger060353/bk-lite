...
1. **画布引擎**:基于 `@antv/x6` + `react-shape` + `selection` + `transform` 插件,实现 zoom/pan、节点拖动、端口磁铁建连线、节点 resize、选中态。**不引入** `minimap` 插件(P0 不做缩略图,见 §2.3;且 X6 minimap 在小屏体验较差)。
...
产品定位是「关键路径值班屏」:一张画布放用户选出的核心设备,不是把 WeOps 已监控网络设备全部铺上。

**推荐规模**:约 50 个画布节点、约 80 条连线。这是交互和运行态刷新按当前实现能舒适工作的量级,也是不做 MiniMap、不做自动布局的前提。

**不是硬上限**:保存、绘制、节点库都不按节点数或连线数拦截。超过推荐规模(例如画布上两三百台)可以保存,但运行态按节点/连线分别请求 WeOps,刷新和编辑会变慢,不作为当前交付承诺。

节点库一次性拉 WeOps 已监控设备全量;库内两三百台可以接受。卡顿风险主要在「已经铺到画布上的节点和连线」,不在侧栏列表本身。
...
保存时不对节点数、连线数设上限(见 §2.3)。推荐规模下 view_sets JSON 反序列化 < 10ms。
...
- ✅ 已确认:约 50 节点 / 80 连线是推荐规模与舒适工作量级,不是保存或绘制的硬上限;见 §2.3。
