...
| 告警与事件 | 查看生命周期，空单可认领 / 分派，当前处理人可转派；空单或当前处理人可手工关闭 | 告警状态 active/recovered/closed；事件不可变；认领 / 分派 / 转派写 `claimed` / `assigned` / `reassigned` 事件；手工分派与转派对系统用户渠道通知新处理人；`my_alert` 只筛处理人，列表与分布图同一套；分布图不含认领 / 分派 / 转派 | Beta |
