### Overview
Collects RocketMQ Broker version, listen port, install path, cluster and NameServer information via a script (JOB), and syncs them to CMDB in a standardized form. Collection is read-only and does not modify any target configuration.
...
- Applies to RocketMQ instances identifiable by script. Directory layout and configuration syntax may differ across major versions; use actual trial-collection results as the reference.
...
2. **Collection account and permissions (required only for SSH remote)**
   - Must be able to read the `broker.conf` configuration file.
   - Must be able to run `mqadmin`.
3. **Target dependencies**
   - RocketMQ is deployed and started.
   - Java runtime is installed on the target, and `mqadmin` is available.

### Procedure
#### Step 0: Entry Point
1. Go to "CMDB → Management → Auto Discovery → Collection → Professional Collection".
2. Select the **RocketMQ** plugin and click "Add Task".
...
| port | Listen port (`broker.conf` listenPort, default `10911`) |
| install_path | Install path |
| configfile | Configuration file path (`broker.conf`) |
...
> Note: related fields may be empty when the `broker.conf` path is non-standard or `mqadmin` is unavailable. This plugin is **BETA / Untested**; verify results before putting it into use.
