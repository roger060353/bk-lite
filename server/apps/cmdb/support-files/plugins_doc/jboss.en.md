### Overview
Collects JBoss / WildFly instance version, role, and configuration via a script (listen port, install path, JVM parameters, configuration file, etc.) and syncs them to CMDB in a standardized form. Collection is read-only and does not modify any target configuration.
...
- Compatible with mainstream JBoss / WildFly versions; verify on the target instance first.
...
2. **Collection account and permissions (required only for SSH remote)**
   - Need permission to read the configuration file (`standalone.xml`) and to run `standalone.sh`.
3. **Target dependencies**
   - Java runtime is installed on the target host.
   - A running JBoss / WildFly process exists on the target host.

### Procedure
#### Step 0: Entry Point
1. Go to "CMDB → Management → Auto Discovery → Collection → Professional Collection".
2. Select the **JBoss** plugin and click "Add Task".
...
| port | HTTP listen port (`standalone.xml`, default `8080`) |
| install_path | Install directory (`-Djboss.home.dir`) |
| version | JBoss / WildFly version |
| jvm_xms | JVM initial heap size (`-Xms`) |
| jvm_xmx | JVM max heap size (`-Xmx`) |
| role | Running role (standalone / host-controller / server) |
| config_file | Configuration file path (`standalone.xml`) |

> Note: `port` is parsed from `standalone.xml`, default `8080`; the actual value follows the configuration. `jvm_xms` and `jvm_xmx` are extracted from process startup parameters and may be empty when not set explicitly.
