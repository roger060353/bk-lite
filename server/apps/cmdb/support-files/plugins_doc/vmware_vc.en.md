...
- After saving and running, check the `Added / Updated / Deleted` summary in the task details. In CMDB you should be able to query `vmware_vc` and its ESXi, VM, and datastore instances.
- If the inventory is incomplete, the account usually was not granted on the vCenter root object, or "Propagate to children" was not checked. Recollect after fixing.


### Credential Fields
- `username`: vCenter login username. Create a dedicated read-only account (for example `svc-cmdb@vsphere.local`); do not reuse an administrator.
- `password`: Password for the account above. Encrypted at rest and injected as an environment variable when dispatched.
- `port`: vCenter API port, default `443`.
- `sslVerify`: Whether to verify the HTTPS certificate; can be temporarily disabled for self-signed certificates when the access point does not have the trust chain.


### Collected Data (Field Dictionary)
Note: `MOID` in the tables below means Managed Object ID, the unique identifier of an object inside vCenter, generated automatically by vCenter.
