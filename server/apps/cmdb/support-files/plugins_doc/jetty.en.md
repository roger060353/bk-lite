### Overview
Collects Jetty instance version, listen port, install directory, JVM/thread-pool parameters, and deployment directory via a script (JOB), and syncs them to CMDB in a standardized form. Collection is read-only and does not modify any target configuration.
...
- Applies to Jetty instances identifiable by script. Directory layout and configuration syntax may differ across major versions; use actual trial-collection results as the reference.
...
2. **Collection account and permissions (required only for SSH remote)**
   - Must be able to read the Jetty home directory and configuration files such as `jetty-threadpool.xml`.
3. **Target dependencies**
   - Jetty is deployed and started.
   - Java runtime is installed on the target.

### Procedure
#### Step 0: Entry Point
1. Go to "CMDB → Management → Auto Discovery → Collection → Professional Collection".
2. Select the **Jetty** plugin and click "Add Task".
...
| port | Listen port |
| jetty_home | Jetty install directory |
| version | Jetty version |
| bin_path | Executable/startup-script path |
| monitored_dir | Deployment directory (`webapps`) |
| java_path | Java executable path |
| java_version | JDK version |
| java_vendor | JDK vendor |
| conf_path | Configuration file path |
| war_name | Deployed WAR package name |
| jvm_para | JVM startup parameters |
| max_threads | Thread-pool max threads |

> Note: related fields may be empty when the configuration file path is non-standard or the corresponding items are not configured. This plugin is **BETA / Untested**; verify results before putting it into use.
