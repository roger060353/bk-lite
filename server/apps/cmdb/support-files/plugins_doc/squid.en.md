### Overview
Collects Squid instance version and key configuration via a script (listen port, install path, configuration file, cache directory, log path, etc.) and syncs them to CMDB in a standardized form. Collection is read-only and does not modify any target configuration.
...
- Compatible with mainstream Squid versions; verify on the target instance first.
...
2. **Collection account and permissions (required only for SSH remote)**
   - Need permission to read the Squid configuration file (`squid.conf`).
3. **Target dependencies**
   - A running Squid process exists on the target host.
   - Can run `squid -v` to get version information.

### Procedure
#### Step 0: Entry Point
1. Go to "CMDB → Management → Auto Discovery → Collection → Professional Collection".
2. Select the **Squid** plugin and click "Add Task".
...
| port | Listen port (`http_port`, default `3128`) |
| version | Squid version (`squid -v`) |
| install_path | Install path |
| config_file_path | Configuration file path (specified by `-f`, or `/etc/squid/squid.conf`) |
| cache_dir | Cache directory |
| access_log | Access log path |
| error_log | Error log path (`cache_log`) |
| visible_hostname | Externally visible hostname |

> Note: `config_file_path` prefers the configuration file specified by the startup `-f` option, and falls back to `/etc/squid/squid.conf` when not specified. `cache_dir`, `access_log`, `error_log`, and `visible_hostname` may be empty when the corresponding items are not configured.
