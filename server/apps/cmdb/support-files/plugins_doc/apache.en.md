### Overview
Collects Apache HTTP Server (httpd) instance version, listen port, executable/configuration paths, document root, and log paths via a script (JOB), and syncs them to CMDB in a standardized form. Collection is read-only and does not modify any target configuration.
...
- Applies to Apache httpd instances identifiable by script. Directory layout (such as `/etc/httpd` vs `/etc/apache2`) and configuration syntax may differ across distributions; use actual trial-collection results as the reference.
...
2. **Collection account and permissions (required only for SSH remote)**
   - Must be able to read the main configuration file (`/etc/httpd/conf/httpd.conf` or `/etc/apache2/apache2.conf`).
   - Must be able to run `httpd -v` to get the version.
3. **Target dependencies**
   - Apache httpd is deployed and started.
   - The target has common commands such as `netstat`.

### Procedure
#### Step 0: Entry Point
1. Go to "CMDB → Management → Auto Discovery → Collection → Professional Collection".
2. Select the **Apache** plugin and click "Add Task".
...
| port | Listen port |
| version | Apache version (`httpd -v`) |
| httpd_path | httpd executable path |
| httpd_conf_path | Main configuration file path |
| doc_root | Document root (`DocumentRoot`) |
| error_log | Error log path |
| custom_log | Access log path |
| include | Configuration paths referenced by include directives |

> Note: related fields may be empty when the configuration file path is non-standard or the corresponding directives are not configured. This plugin is **BETA / Untested**; verify results before putting it into use.
