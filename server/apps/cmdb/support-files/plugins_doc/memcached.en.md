### Overview
Parses a running Memcached process via a script (JOB), collects version, listen port, max connections, and cache size, and syncs them to CMDB in a standardized form. Collection is read-only and does not modify any target configuration.
...
- Applies to Memcached instances identifiable by script. Startup parameters and output may differ across major versions; use actual trial-collection results as the reference.
...
3. **Target dependencies**
   - Memcached is deployed and started, and the process runs with parameters such as `-p`/`-m`.

### Procedure
#### Step 0: Entry Point
1. Go to "CMDB → Management → Auto Discovery → Collection → Professional Collection".
2. Select the **Memcached** plugin and click "Add Task".
...
| port | Listen port (startup parameter `-p`) |
| version | Memcached version (`memcached -V`) |
| install_path | Install path |
| maxconn | Max connections (startup parameter `-c`) |
| cachesize | Cache size (startup parameter `-m`, unit MB) |
| user_name | Process owner |

> Note: related fields may be empty when the process does not have the corresponding startup parameters, or `/proc/<pid>/cmdline` cannot be read. This plugin is **BETA / Untested**; verify results before putting it into use.
