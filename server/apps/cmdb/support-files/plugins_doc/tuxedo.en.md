### Overview
Collects Tuxedo instance version, patch level, listen port, install path, domain information, and thread parameters via a script (JOB), and syncs them to CMDB in a standardized form. Collection is read-only and does not modify any target configuration.
...
- Applies to Tuxedo instances identifiable by script. Directory layout and configuration syntax may differ across major versions; use actual trial-collection results as the reference.
...
2. **Collection account and permissions (required only for SSH remote)**
   - Must be able to run `tmadmin`.
   - Must be able to read the `TUXCONFIG` configuration file.
...
3. **Target dependencies**
   - Tuxedo is deployed and started.
   - The target has the `tmadmin` binary.

### Procedure
#### Step 0: Entry Point
1. Go to "CMDB → Management → Auto Discovery → Collection → Professional Collection".
2. Select the **Tuxedo** plugin and click "Add Task".
...
| conf_file | Configuration file path (`TUXCONFIG` or install_path/ubbconfig) |
| version | Tuxedo version (`tmadmin -v`) |
| patch_level | Patch level |
| domainid | Domain ID |
| ipckey | IPC Key (Tuxedo inter-process communication identifier) |
| lmid | Logical Machine ID |
| maxdispatchthreads | Max dispatch threads |
| mindispatchthreads | Min dispatch threads |

> Note: `TUXCONFIG` is an environment variable/file path pointing to Tuxedo configuration. Related fields may be empty when `tmadmin` is unavailable, the `TUXCONFIG` path is non-standard, or `/proc/<pid>/cmdline` cannot be read. This plugin is **BETA / Untested**; verify results before putting it into use.
