...
def test_classify_hits_stamps_snapshot_without_writing_ci(mocker):
...
def test_rematch_soid_classifies_from_oid_mapping(mocker):
    ...
    assert {hit.cmdb_model_id for hit in ScanHit.objects.filter(execution=execution)} == {""}
    assert ScanHit.objects.get(host="10.0.1.11").snapshot.get("device_type") == "firewall"
    assert ScanHit.objects.get(host="10.0.1.11").snapshot.get("model") == "FW1"
    assert ScanHit.objects.get(host="10.0.1.11").inst_uuid == ""
    cannula.assert_not_called()
...
def test_rematch_soid_skips_already_classified(mocker):
    execution, _family_run, hits = _execution(hosts=["10.0.1.11", "10.0.1.12"])
    hits[0].snapshot = {**(hits[0].snapshot or {}), "device_type": "switch"}
    hits[0].save(update_fields=["snapshot"])
    ...
    assert result["classified"] == 1
    hits[0].refresh_from_db()
    assert hits[0].snapshot.get("device_type") == "switch"
    second = ScanHit.objects.get(host="10.0.1.12")
    assert second.snapshot.get("device_type") == "router"
...
def test_rematch_soid_stamps_empty_soid_hits_when_hit_ids_given(mocker):
    ...
    assert first.cmdb_model_id == ""
    assert first.snapshot.get("device_type") == "switch"
    assert first.snapshot.get("sysobjectid") == UNKNOWN_OID
    assert first.inst_uuid == ""
    cannula.assert_not_called()
    assert second.soid == ""
    assert second.cmdb_model_id == ""
