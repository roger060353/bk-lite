...
class AlertRecoveryChecker:
    ...
    def check_and_recover_alert(alert: Alert) -> bool:
        ...
        for created_event in created_events:
            ...
            recovery_events = recovery_by_match_key.get(build_recovery_match_key(created_event), [])

            created_event_time = AlertRecoveryChecker._get_created_event_time(created_event)

            # 检查是否有事件时间更晚的 RECOVERY/CLOSED 事件
            has_later_recovery = any(AlertRecoveryChecker._get_recovery_event_time(r) > created_event_time for r in recovery_events)
        ...
        if alert.is_session_alert and alert.session_status == SessionStatus.OBSERVING:
            ...
            alert.save(update_fields=["status", "session_status", "updated_at", "closed_at"])
            logger.info(
