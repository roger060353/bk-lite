...
def test_create_and_list_rule(superuser_client):
    ...
    payload = {
        ...
        "action_type": "job",
        "action_config": {"script_id": 1, "target_binding": {"source": "node_mgmt", "host_field": "labels.ip"}, "param_bindings": []},
    }
...
def test_patch_toggle_active(superuser_client):
    ...
    superuser_client.cookies["current_team"] = "1"
    resp = superuser_client.patch(f"/api/v1/alerts/api/action_rule/{rule.id}/", data={"is_active": False}, format="json")
