...
from apps.alerts.common.notification_target import resolve_notification_target
...
def _find_assignment_for_alert(alert: Alert) -> Optional[AlertAssignment]:
    ...
    表是 OneToOne 主键，最多一条。
    """
    task = AlertReminderTask.objects.filter(alert=alert).select_related("assignment").first()
...
def notify_alert_recovered(alert: Alert) -> bool:
    ...
    target = (assignment.config or {}).get("notification_target") if isinstance(assignment.config, dict) else None
    receivers = resolve_notification_target(target, assignment.personnel)
    if not receivers:
        logger.warning(
    ...
    ok = enqueue_notifications(
        params,
        idempotency_key=f"recovery-notify:{alert.alert_id}",
    )
    logger.info(
