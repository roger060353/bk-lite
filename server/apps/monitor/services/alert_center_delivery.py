...
def enqueue_alert_center_deliveries(
    ...
    *,
    ...
    with transaction.atomic():
        ...
        instance_identity_map = notifier._build_instance_identity_map(target_alerts)
        for original in target_alerts:
            alert = locked_by_id.get(original.id, original)
            base_payload = notifier._build_alert_center_payload(alert, action, operator, reason, instance_org_map, instance_identity_map)
