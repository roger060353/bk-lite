...
本插件通过 Telegraf `inputs.prometheus` 从 Stargazer 拉取 Meraki Dashboard API v1 指标，覆盖无线 AP 丢包与以太网状态。组织核验、以太网状态或丢包接口失败时导出 `meraki_wireless_connect_status=0`。`ap_count` 按以太网状态与丢包数据中的 serial 去重统计。以太网 `link_up` 优先使用端口上的 `status` / `linkStatus` / `connected`；没有明确字段时，再按 `linkNegotiation.speed > 0` 或 `duplex` 为 `full`/`half` 判断。
