"""HP-UX OS 监控：远程 ksh 包装与 Prometheus 映射。"""
...
HPUX_SCRIPT_PATH = Path(__file__).parent / "scripts" / "hpux" / "os_monitor.ksh"
HPUX_COLLECT_EOF = "STARGAZER_HPUX_COLLECT_EOF"
HPUX_KSH_PREFIX = "LC_ALL=C LANG=C /usr/bin/ksh"

COMMAND_EXECUTE_TIMEOUT = int(os.getenv("COMMAND_EXECUTE_TIMEOUT", "900"))


def load_hpux_monitor_script() -> str:
    return HPUX_SCRIPT_PATH.read_text(encoding="utf-8")


def wrap_ksh_collect(script_body: str | None = None) -> str:
    body = script_body if script_body is not None else load_hpux_monitor_script()
    return f"{HPUX_KSH_PREFIX} <<'{HPUX_COLLECT_EOF}'\n{body.rstrip()}\n{HPUX_COLLECT_EOF}\n"
...
def _append_hpux_diskio(lines: list[str], diskios: Any, base_labels: str, timestamp: int) -> None:
    ...
    for diskio in diskios:
        ...
        _append_gauge(
            ...
            interval_r,
            timestamp,
            "Disk read bytes from sar/iostat interval sample",
        )
        _append_gauge(
            lines,
            "diskio_write_bytes",
            ...
            interval_w,
            timestamp,
            "Disk write bytes from sar/iostat interval sample",
        )
        if total_r is not None:
            _append_gauge(
                ...
                total_r,
                timestamp,
                "Disk read bytes counter when present",
            )
        if total_w is not None:
            _append_gauge(
                ...
                total_w,
                timestamp,
                "Disk write bytes counter when present",
            )
        _append_gauge(lines, "disk_tm_act", diskio_labels, diskio.get("tm_act", 0), timestamp, "HP-UX sar -d %busy disk busy")


# JSON → Prometheus lock-in: scripts/hpux/samples/os_monitor.sample.json
def parse_hpux_metrics_to_prometheus(
    data: dict[str, Any],
    instance_id: str,
    os_type: str,
    timestamp: int,
    *,
    ...
    if os_info:
        ...
        _append_gauge(lines, "os_info", info_labels, 1, timestamp, "HP-UX uname version and architecture")
    ...
    _append_hpux_diskio(lines, data.get("diskio"), base_labels, timestamp)
