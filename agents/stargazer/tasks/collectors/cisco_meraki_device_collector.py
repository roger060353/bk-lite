...
def _connect_status_output(organization_id: str, status: int) -> str:
    metric_dict = {
        (organization_id, "meraki_device_inventory"): {
            "meraki_device_connect_status": _gauge(status),
        }
    }
    ...
    async def collect(self) -> str:
        ...
        try:
            async with MerakiDashboardClient(
                ...
                devices = await _require_list(
                    client,
                    f"/organizations/{organization_id}/devices",
                    {"perPage": 1000},
                )
                availabilities = await _require_list(
                    client,
        ...
        for device in devices:
            ...
            metric_dict[(serial, "meraki_device")] = {"meraki_device_availability_status": _dim_gauge(dims, AVAILABILITY_STATUS[status_key])}
        for row in uplinks:
...
def _required_params(collector: BaseCollector) -> dict[str, Any]:
    ...
    try:
        ...
        timeout = 60.0
    try:
        timespan = int(collector.params.get("uplink_timespan") or collector.params.get("timespan") or 300)
    except (TypeError, ValueError):
        timespan = 300
    return {
        "api_key": api_key,
