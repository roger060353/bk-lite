...
INACTIVE=$(_sysctl_n vm.stats.vm.v_inactive_count)
CACHE=$(_sysctl_n vm.stats.vm.v_cache_count)
...
MEM_AVAILABLE=$(${AWK} -v t="${MEM_TOTAL}" -v f="${FREE_PG}" -v i="${INACTIVE}" -v c="${CACHE}" -v p="${PAGE_SIZE}" 'BEGIN {
  a = (f+0+i+0+c+0)*p
  if (t+0 > 0 && a > t) a = t
  if (a < 0) a = 0
  printf "%.0f", a
