"""FreeBSD OS 监控：远程 /bin/sh 包装与 Prometheus 映射。"""
...
FREEBSD_SCRIPT_PATH = Path(__file__).parent / "scripts" / "freebsd" / "os_monitor.sh"
FREEBSD_COLLECT_EOF = "STARGAZER_FREEBSD_COLLECT_EOF"
FREEBSD_SH_PREFIX = "LC_ALL=C LANG=C /bin/sh"

COMMAND_EXECUTE_TIMEOUT = int(os.getenv("COMMAND_EXECUTE_TIMEOUT", "900"))


def load_freebsd_monitor_script() -> str:
    return FREEBSD_SCRIPT_PATH.read_text(encoding="utf-8")


def wrap_sh_collect(script_body: str | None = None) -> str:
    body = script_body if script_body is not None else load_freebsd_monitor_script()
    return f"{FREEBSD_SH_PREFIX} <<'{FREEBSD_COLLECT_EOF}'\n{body.rstrip()}\n{FREEBSD_COLLECT_EOF}\n"
...
def _append_freebsd_diskio(lines: list[str], diskios: Any, base_labels: str, timestamp: int) -> None:
...
# JSON → Prometheus lock-in: scripts/freebsd/samples/os_monitor.sample.json
def parse_freebsd_metrics_to_prometheus(
    data: dict[str, Any],
    instance_id: str,
    os_type: str,
    timestamp: int,
    *,
    ...
    if os_info:
        ...
        _append_gauge(lines, "os_info", info_labels, 1, timestamp, "FreeBSD uname version and architecture")
    ...
    _append_freebsd_diskio(lines, data.get("diskio"), base_labels, timestamp)
